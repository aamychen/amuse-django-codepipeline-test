...
def default():
    ...
    return CustomerIO(site_id=site_id, api_key=api_key)
...
class CustomerIO(object):
    ...
    def setup_base_url(self):
        ...
        self.base_url = template.format(
            host=self.host.strip('/'), port=self.port, prefix=self.url_prefix.strip('/')
        )

    def setup_connection(self):
        self.http = Session()
        # Retry request a number of times before raising an exception
        self.http.mount('https://', HTTPAdapter(max_retries=self.retries))
    ...
    def get_device_query_string(self, customer_id):
        ...
        return '{base}/customers/{id}/devices'.format(
            base=self.base_url, id=customer_id
        )
...
Last caught exception -- {klass}: {message}
            """.format(
                klass=type(e), message=e, count=self.retries
            )
    ...
    def pageview(self, customer_id, page, **data):
        ...
        url = self.get_event_query_string(customer_id)
        post_data = {'type': "page", 'name': page, 'data': data}
        self.send_request('POST', url, post_data)
    ...
    def backfill(self, customer_id, name, timestamp, **data):
        ...
        post_data = {'name': name, 'data': data, 'timestamp': timestamp}
    ...
    def suppress(self, customer_id):
        ...
        self.send_request(
            'POST',
            '{base}/customers/{id}/suppress'.format(base=self.base_url, id=customer_id),
            {},
        )
    ...
    def unsuppress(self, customer_id):
        ...
        self.send_request(
            'POST',
            '{base}/customers/{id}/unsuppress'.format(
                base=self.base_url, id=customer_id
            ),
            {},
        )
