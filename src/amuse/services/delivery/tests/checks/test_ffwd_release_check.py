...
from amuse.services.delivery.checks import FFWDReleaseCheck
...
class TestFFWDReleaseCheck(TestCase):
    @responses.activate
    def setUp(self):
        add_zendesk_mock_post_response()
    ...
    def test_check_passing(self):
        check = FFWDReleaseCheck(
            release=self.release, store=self.spotify_store, operation='takedown'
        )
        self.assertTrue(check.passing())

    @mock.patch("releases.models.SongFile.file.field.storage", FileSystemStorage())
